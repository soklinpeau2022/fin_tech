import 'package:fin_tech/service/data_year.dart';
...
class _WeekState extends State<Year> {
  late Future<List<SpendingDetails>> _spendingDataFuture;

  @override
  void initState() {
    super.initState();
    _spendingDataFuture = DataServiceYear.loadSalesData();
  }
                                ...
                                if (snapshot.connectionState == ConnectionState.waiting) {
                                  ...
                                  return Column(
                                    ...
                                    children: [
                                        ...
                                        child:  SfCartesianChart(
                                          ...
                                          series: <ChartSeries>[
                                            LineSeries<SpendingDetails, String>(
                                              ...
                                              xValueMapper: (SpendingDetails details, _) => details.date?.year.toString(),
                                      ...
                                      Container(
                                          width: double.infinity,
                                          height: MediaQuery.of(context).size.height*0.643 ,
                                          decoration: BoxDecoration(
                                           // borderRadius: BorderRadius.circular(8),
                                          ),
                                          child: ListView.builder(
                                              shrinkWrap: true,
                                              itemCount: snapshot.data?.length ?? 0, // Use the length of the loaded sales data or 0 if it's null
                                              itemBuilder: (BuildContext context, int index) {
                                                final saleDetails = snapshot.data?[index]; // Get the sales data at the current index or null if it doesn't exist
                                                return Padding(
                                                  padding: EdgeInsets.all(8.0),
                                                  child: saleDetails != null ? Cards(
                                                    calendar:'${saleDetails.date?.year}',
                                                    price:'\$${saleDetails.price?.toStringAsFixed(2)}',
                                                    spendto:'',
                                                    date: '${saleDetails.date?.year}',
